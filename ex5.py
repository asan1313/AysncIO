...
async def main():
    ...
    # Gather Tasks..run multiple cproutines conc and collect all results
    #if one fails, it cancels others..unless return_exceptions=True
    tasks = [asyncio.create_task(fetch_data(i)) for i in range(1, 3)]
    results = await asyncio.gather(*tasks)
    print(f"Task Results: {results}")

    # Task Group..one task fails , cancels rest and gives error
